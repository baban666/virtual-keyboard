            ...
            button.addEventListener('click', function(e) {
                ...
                switch (Number(keyCode)) {
                    ...
                    case 46:
                        ...
                        lastPosition = getCharPosition();
                        screen.updateScreen(text);
                    }else {
                        if(lastPosition ==  text.length){
                            screen.updateScreen(text);
                        }else {
                            text.splice(lastPosition, 1);
                            screen.updateScreen(text);
                        }
                    };
                        const inputDel = document.getElementById('kp');
                    ...
                    default:
                        if (getCharPosition() != 0){
                            if(isDigit){
                                ...
                                lastPosition = getCharPosition();
                                (!(e.shiftKey && getCapsLock()) && (e.shiftKey || getCapsLock())) ? text.splice(getCharPosition(), 0, elem.innerText.toUpperCase()) : text.splice(getCharPosition(), 0, elem.innerText.toLowerCase());
                                screen.updateScreen(text);
                            }
                        }else {
                            if(isDigit){
